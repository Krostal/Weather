        ...
        let timePeriodForNextDays = timePeriod.filter {
            if let time = $0.time {
                if let savedTime = ISO8601DateFormatter().date(from: time) {
                    let formattedTime = dateFormatter.formattedDateToString(date: savedTime, dateFormat: "yyyy-MM-dd")
                    return dateFormatter.formattedDateToString(date: Date(), dateFormat: "yyyy-MM-dd") != formattedTime
                }
            }
            return true
        }
